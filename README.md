...
```bash
registerToken$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
registerToken$ sam local start-api
registerToken$ curl http://localhost:3000/
```
...
```bash
registerToken$ sam logs -n HelloWorldFunction --stack-name registerToken --tail
```
...
```bash
aws cloudformation delete-stack --stack-name registerToken
```
